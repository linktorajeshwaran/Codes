...
import practice3.datastructures.ChainingHashDictionary;
import practice3.datastructures.IterableString;
import practice3.datastructures.MoveToFrontDictionary;
import practice3.interfaces.ITrieMap;
...
public class MarkovTextGenerator {
    ...
    public static IDictionary<IterableString, Integer> newInnerTrieMap() {
		/*
		 * return new TrieMap<>((IDeque<Character> x) -> { char[] chars = new
		 * char[x.size()]; for (int i = 0; i < chars.length; i++) { chars[i] =
		 * x.peekFront(); x.addBack(x.removeFront()); } return new IterableString(new
		 * String(chars)); });
		 */
    	return new TrieMap();
    }
